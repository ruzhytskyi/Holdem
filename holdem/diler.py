class Diler(object):
    ...
    def __check_straight_flush__(self, cards):
        ...
        if len(scards) < 5:
            ...
            return __check_straight__(scards)
    ...
    def __check_straight__(self, cards):
        ...
        for i in reversed(range(len(scards) - 4)):
            ...
            if __is_straight__(comb) == True:
                return comb

    def __check_three_of_kind__(self, cards):
        """Returns a list of 5 cards from given list that form three of a kind or None otherwise"""
        scards = __same_rank__(cards)[0]
        if len(scards) != 3:
            return None
        else:
